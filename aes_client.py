...
plaintext = input("Enter the plaintext message (Hexadecimal): ").strip()

key = input("Enter the key for encryption: ").strip()

keys = roundkeygen(str2hex(key))

cipher = encrypt(str2hex(plaintext), keys)
print(f"Encrypted message sent to server: {cipher}")
client_socket.sendall(cipher.encode())

encrypted_reply = client_socket.recv(1024).decode()
print(f"Encrypted reply received from server: {encrypted_reply}")

reply = decrypt(encrypted_reply, keys)
print(f"Decrypted reply from server: {hex2str(reply)}")
